...
makeTree = function(where2,where3){
  ...
  prp(fit,uniform=T,fallen.leaves=TRUE,type=2,yesno = TRUE, extra = 2, box.col = c(adjustcolor('blue',0.7),adjustcolor('red',0.7))[fit$frame$yval], branch.col='black')
  return(fit)
}
...
error.types = matrix(c(tot.errors.gop,tot.errors.dem), byrow = TRUE, nrow = 2)
colnames(error.types) = names(groups)
barplot(error.types, beside = TRUE, col = c('red','blue'), main = "Error Rates from RPart per Region")
...
error.types.knn = matrix(c(tot.errors.gop.knn,tot.errors.dem.knn), byrow = TRUE, nrow = 2)
colnames(error.types.knn) = names(groups)
barplot(error.types.knn, beside = TRUE, col = c('red','blue'), main = "Error Rates from Knn per Region")
legend('topright', fill = c('red','blue'),
       legend = c('Error in Favor of Bush','Error in Favor of Kerry'),
       bty = 'n', cex = 0.7)
